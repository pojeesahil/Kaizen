...
- Single Source of Truth & Dynamic Derivation: Never plan tasks that hardcode arbitrary spatial coordinates, entity positions, or boundary magic numbers. Entity spawn positions, grid sizes, and viewport dimensions MUST be parsed or queried dynamically from the central data model or layout structure (e.g. `layout.getPlayerSpawn()`, `board.getInitialPositions()`, `gridWidth * cellSize`).
- Self-Describing Data Structures: Map, maze, or board definitions must use self-describing markers or schema tokens (e.g. wall, path, player spawn, enemy spawn) so dependent entity tasks query the model directly rather than guessing row/column indices.
...
- Vertical Slice Protocol: Structure tasks as runnable vertical milestones, NOT horizontal layers. Each task must produce a working, testable application state by cutting through all necessary layers (data model, logic, rendering, entrypoint). For example, instead of 'Task 1: Create all models, Task 2: Create all renderers, Task 3: Wire entrypoint', structure as 'Task 1: Core data model + minimal renderer + entrypoint that displays a static screen, Task 2: Add player entity + input handling + movement visible on screen, Task 3: Add enemies + collision + game loop, Task 4: Add HUD/score/menus + polish'. Each milestone must be independently runnable and verifiable before the next milestone builds on top of it.
